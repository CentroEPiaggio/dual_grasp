...
bool ReGrasp::call_simple_grasp_task_2(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res){
    
    // Checking the request for correctness
    if(!req.data){
        ROS_WARN("Did you really want to call the simple grasp task service with data = false?");
        res.success = true;
        res.message = "The service call_simple_grasp_task_2 done correctly with false request!";
        return true;
    }
    ...
    // 5) Going to post grasp pose
    if(!this->panda_softhand_client_2.call_slerp_service_2(post_grasp_pose, grasp_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok){
        ROS_ERROR("Could not plan to the specified grasp pose.");
        res.success = false;
        res.message = "The service call_simple_grasp_task_2 was NOT performed correctly!";
        return false;
...
// 5) Going to home grasp pose
    if(!this->panda_softhand_client_2.call_slerp_service_2(home_grasp_pose, post_grasp_pose, false, this->tmp_traj, this->tmp_traj) || !this->franka_ok){
        ROS_ERROR("Could not plan to the specified grasp pose.");
        res.success = false;
        res.message = "The service call_simple_grasp_task_2 was NOT performed correctly!";
        return false;
    ...
    res.success = true;
    res.message = "The service call_simple_grasp_task_2 was correctly performed!";
    return true;
}
